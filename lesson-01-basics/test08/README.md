...
`comparisons` 是小写字段，外部测试包读不到，只能走 `Comparisons()` 方法。这正是 test06 的可见性规则第一次用在有意义的地方：内部计数器不对外暴露写入能力。
...
```bash
go test ./lesson-01-basics/test08/...
go test -v -run TestBridge ./lesson-01-basics/test08/roster   # 看桥接测试打印的数字
```
...
跑一遍 `go test -v -run TestBridge ./lesson-01-basics/test08/roster`，把输出留着。三件事：
